...
import { Location, Currency, OrganizationService, ImportJobStatus, ExportJobStatus, ImportExportHistory, ImportExportHistoryResponse } from '../../../core/services/organization.service';
...
export class LocationsComponent implements OnInit, OnChanges, OnDestroy {
  ...
  // Filters
  filters: any = { createdFrom: '', createdTo: '', status: '', country: '', city: '' };
  filterFormData: any = { createdFrom: '', createdTo: '', status: '', country: '', city: '' };
  showFilters = false;
  searchQuery = '';
  ...
  cities: string[] = [];
  currencies: Currency[] = [];
  locationTypes: string[] = ['HEADQUARTERS', 'BRANCH', 'WAREHOUSE', 'RETAIL', 'OFFICE', 'DISTRIBUTION_CENTER'];
  ...
  initializeDropdownStates(): void {
    const fields = ['status', 'filter-status', 'filter-country', 'filter-city', 'items-per-page', 'currency', 'locationType', 'parentLocation'];
  ...
  getOptionsForField(fieldName: string): string[] {
    switch (fieldName) {
      ...
      case 'items-per-page':
        return ['10', '25', '50', '100'];
      case 'currency':
        return this.currencies.map(c => c.code || c.name).filter(Boolean);
      case 'locationType':
  ...
  loadData(): void {
    ...
    const sub = this.orgService.getLocations(params).subscribe({
      next: (res: any) => {
        const items = Array.isArray(res?.items) ? res.items : (Array.isArray(res) ? res : []);
        this.items = items.map((item: Location) => ({
          ...item,
          showDropdown: false
        }));
        this.totalItems = res?.totalCount || items.length;
        this.totalPages = res?.totalPages || Math.ceil(this.totalItems / this.itemsPerPage);
      ...
      error: () => {
        this.isLoading = false;
        this.notification.error('Error', 'Failed to load locations');
      }
    });
  ...
  loadFilterOptions(): void {
    ...
    this.orgService.getLocationCountries().subscribe({
      next: (countries) => {
    ...
    this.orgService.getLocationCities().subscribe({
      next: (cities) => {
        this.cities = cities;
      },
      error: () => {
    ...
    this.orgService.getCurrencies({ isActive: true, pageSize: 1000 }).subscribe({
      next: (response: any) => {
    ...
    // Load parent locations (all active locations except current one if editing)
    this.orgService.getLocations({ page: 1, pageSize: 1000, isActive: true }).subscribe({
      next: (response: any) => {
        const items = Array.isArray(response?.items) ? response.items : (Array.isArray(response) ? response : []);
        // Filter out current location if editing
        if (this.selectedItem?.id) {
          this.parentLocations = items.filter((loc: Location) => loc.id !== this.selectedItem?.id);
        } else {
          this.parentLocations = items;
        }
      },
      error: () => {
  ...
  clearFilters(): void {
    this.searchQuery = '';
    this.filters = { createdFrom: '', createdTo: '', status: '', country: '', city: '' };
    this.filterFormData = { createdFrom: '', createdTo: '', status: '', country: '', city: '' };
  ...
  updatePagination(): void {
    // Server-side pagination, so no need to slice data
    // Data is already paginated from the backend
  }

  onPageChange(page: number): void {
    this.currentPage = page;
    this.loadData(); // Fetch new page from backend
  }

  onItemsPerPageChange(): void {
    this.currentPage = 1;
    this.itemsPerPage = this.itemsPerPageFormData;
    this.loadData(); // Fetch new page size from backend
  }
  ...
  openEditDialog(item: Location): void {
    ...
    this.locationForm.patchValue({
      ...
      locationType: item.locationType || 'BRANCH',
  ...
  openViewDialog(item: Location): void {
    // Load full location details with metadata
    const sub = this.orgService.getLocation(item.id).subscribe({
      next: (fullLocation) => {
        this.selectedItem = fullLocation;
  ...
  openEditDialogFromView(item: Location): void {
    this.showViewDialog = false;
    this.openEditDialog(item);
  }

  exportLocationDetailsAsPDF(): void {
    if (!this.selectedItem) {
      this.notification.warning('Warning', 'No location selected to export');
      return;
    }

    const modalBody = document.querySelector('.modal-overlay .modal .modal-body') as HTMLElement;
    const locationDetailsCards = document.querySelector('.modal-overlay .modal .profile-form') as HTMLElement;

    if (!modalBody || !locationDetailsCards) {
    ...
    setTimeout(() => {
      html2canvas(locationDetailsCards, {
        scale: 2,
        useCORS: true,
        allowTaint: true,
        ...
        scrollX: 0,
        scrollY: 0,
        windowWidth: locationDetailsCards.scrollWidth,
        windowHeight: locationDetailsCards.scrollHeight
      }).then(canvas => {
        ...
        pdf.save(`location-details-${this.selectedItem?.name.replace(/\s+/g, '-').toLowerCase()}.pdf`);
  ...
  openDeleteDialog(items: Location[]): void {
  ...
  cloneSelected(): void {
    ...
    if (selectedIds.length === 0) {
      this.notification.warning('Warning', 'Please select at least one location to clone');
      return;
    }

    this.notification.info('Info', 'Cloning locations...');
    this.orgService.cloneMultipleLocations(selectedIds).subscribe({
      next: (response) => {
        this.notification.success('Success', response.message || `${selectedIds.length} location(s) cloned successfully`);
        this.selectedItems.clear();
        this.resetSortToDefault();
        this.loadData();
      },
      error: (error) => {
        this.notification.error('Error', error.error?.message || 'Failed to clone locations');
      }
    });
  }

  cloneLocation(item: Location): void {
    this.notification.info('Info', 'Cloning location...');
    this.orgService.cloneMultipleLocations([item.id]).subscribe({
      next: (response) => {
        this.notification.success('Success', response.message || 'Location cloned successfully');
        this.resetSortToDefault();
        this.loadData();
      },
      error: (error) => {
        this.notification.error('Error', error.error?.message || 'Failed to clone location');
      }
    });
  }

  closeDropdown(item: Location): void {
  ...
  saveItem(): void {
    ...
    const processedValue = {
      ...formValue,
      latitude: formValue.latitude && formValue.latitude !== '' ? parseFloat(formValue.latitude) : null,
      longitude: formValue.longitude && formValue.longitude !== '' ? parseFloat(formValue.longitude) : null,
      ...
      managerName: formValue.managerName || null,
      locationCode: formValue.locationCode || null,
      locationType: formValue.locationType || 'BRANCH',
      timezone: formValue.timezone || null,
      currency: formValue.currency || null,
      language: formValue.language || null,
      parentLocationId: formValue.parentLocationId || null,
      isDefault: formValue.isDefault || false
    };

    if (this.dialogMode === 'add') {
      this.orgService.createLocation(processedValue).subscribe({
        next: () => {
          this.closeDialogs();
          this.notification.success('Success', 'Location created successfully!');
        ...
        error: (error) => {
          this.notification.error('Error', error.error?.message || 'Failed to create location');
          this.isSubmitting = false;
        }
      });
    } else if (this.selectedItem?.id) {
      this.orgService.updateLocation(this.selectedItem.id, processedValue).subscribe({
        next: () => {
          this.closeDialogs();
          this.notification.success('Success', 'Location updated successfully!');
          this.isSubmitting = false;
          this.resetSortToDefault();
          this.loadData();
          this.loadStatistics();
        ...
        error: (error) => {
          this.notification.error('Error', error.error?.message || 'Failed to update location');
          this.isSubmitting = false;
        }
      });
    }
  ...
  confirmDelete(): void {
    ...
    if (itemsToDelete.length === 1) {
      this.orgService.deleteLocation(itemsToDelete[0]).subscribe({
        next: () => {
          this.selectedItems.clear();
          this.closeDialogs();
          this.notification.success('Success', 'Location deleted successfully!');
          this.resetSortToDefault();
          this.loadData();
          this.loadStatistics();
        },
        error: (error) => {
          this.notification.error('Error', error.error?.message || 'Failed to delete location');
        }
      });
    } else {
      // Bulk delete
      ...
      itemsToDelete.forEach(id => {
        this.orgService.deleteLocation(id).subscribe({
          next: () => {
            deletedCount++;
            if (deletedCount === totalCount) {
              this.selectedItems.clear();
              this.closeDialogs();
              this.notification.success('Success', `${totalCount} locations deleted successfully`);
              this.resetSortToDefault();
              this.loadData();
              this.loadStatistics();
            }
          ...
          error: () => {
            ...
            if (deletedCount === totalCount) {
              this.selectedItems.clear();
              this.closeDialogs();
              this.notification.warning('Warning', 'Some locations could not be deleted');
              this.resetSortToDefault();
              this.loadData();
              this.loadStatistics();
            }
  ...
  exportLocations(format: 'excel' | 'csv' | 'pdf' | 'json'): void {
    ...
    const params: any = {
      ...
      isActive: this.filters.status ? (this.filters.status === 'true') : undefined,
    ...
    this.orgService.startLocationExportAsync(params).subscribe({
      next: (response) => {
  ...
  private pollExportJobStatus(): void {
    ...
    const intervalId = setInterval(() => {
      ...
      this.orgService.getLocationExportJobStatus(this.exportJobId).subscribe({
  ...
  private downloadCompletedExport(jobId: string, format: string): void {
    this.orgService.downloadLocationExport(jobId).subscribe({
      next: (blob: Blob) => {
        ...
        link.href = url;
        link.download = `locations_export_${new Date().toISOString().split('T')[0]}.${extension}`;
  ...
  downloadTemplate(): void {
    this.orgService.getLocationTemplate().subscribe({
      next: (response: Blob) => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = 'locations_import_template.xlsx';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
  ...
  importData(): void {
    ...
    // Start async job
    this.orgService.startLocationImportAsync(formData).subscribe({
      next: (res) => {
  ...
  private pollImportJobStatus(): void {
    ...
    const intervalId = setInterval(() => {
      ...
      this.orgService.getLocationImportJobStatus(this.importJobId).subscribe({
        next: (status: ImportJobStatus) => {
          ...
          if (status.status === 'Completed' || status.status === 'Failed') {
            ...
            if (status.status === 'Completed') {
              this.notification.success('Import Completed', `Imported ${status.successCount} locations${status.skippedCount ? `, skipped ${status.skippedCount}` : ''}${status.errorCount ? `, errors ${status.errorCount}` : ''}.`);
  ...
  downloadErrorReport(): void {
    ...
    this.orgService.getLocationImportErrorReport(this.errorReportId).subscribe({
      next: (response: Blob) => {
  ...
  downloadErrorReportForHistory(historyId: string): void {
    ...
    this.orgService.getLocationImportErrorReport(history.errorReportId).subscribe({
      next: (response: Blob) => {
        const blob = new Blob([response], { type: 'text/csv' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
  ...
  downloadExportFromHistory(historyId: string): void {
    ...
    this.orgService.downloadLocationExport(history.jobId).subscribe({
      next: (blob: Blob) => {
        // Determine file extension based on format
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = history.fileName || `locations_export_${new Date().toISOString().split('T')[0]}.${extension}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
  ...
  loadHistory(): void {
    ...
    this.orgService.getLocationHistory(this.historyType, this.historyPage, this.historyPageSize).subscribe({
      next: (response) => {
  ...
  // Dropdown methods
  isDropdownOpen(item: Location): boolean {
    return !!(item as any).showDropdown;
  }

  isDropdownUpward(item: Location): boolean {
    if (!this.isDropdownOpen(item)) return false;
    const itemIndex = this.paginatedData.findIndex(i => i.id === item.id);
    // If item is in the last 3 rows, show dropdown upward
    return itemIndex >= this.paginatedData.length - 3;
  }

  toggleDropdown(item: Location, event?: MouseEvent): void {
    if (event) event.stopPropagation();
    this.exportDropdownOpen = false;
    this.items.forEach((dataItem: Location) => {
  ...
  toggleActiveStatusAndClose(item: Location): void {
    this.toggleActiveStatus(item);
    (item as any).showDropdown = false;
  }

  deleteItemAndClose(item: Location): void {
    this.openDeleteDialog([item]);
    (item as any).showDropdown = false;
  }

  closeAllDropdowns(): void {
    this.items.forEach((item: Location) => {
      (item as any).showDropdown = false;
    });
  ...
  toggleSearchableDropdown(fieldName: string): void {
    ...
    if (this.dropdownStates[fieldName].isOpen) {
      ...
      this.dropdownStates[fieldName].searchTerm = '';
      // Initialize filteredCurrencies for currency dropdown
      if (fieldName === 'currency' && !this.dropdownStates[fieldName].filteredCurrencies) {
        this.dropdownStates[fieldName].filteredCurrencies = [...this.currencies];
      }
      // Initialize filteredLocations for parent location dropdown
      if (fieldName === 'parentLocation' && !this.dropdownStates[fieldName].filteredLocations) {
        this.dropdownStates[fieldName].filteredLocations = [...this.parentLocations];
      }
  ...
  filterOptions(fieldName: string): void {
    ...
    // Special handling for currency dropdown - filter from currencies array
    if (fieldName === 'currency') {
      if (!searchTerm || searchTerm.trim() === '') {
        this.dropdownStates[fieldName].filteredCurrencies = [...this.currencies];
      } else {
        const searchLower = searchTerm.toLowerCase();
        this.dropdownStates[fieldName].filteredCurrencies = this.currencies.filter(c => 
          (c.code && c.code.toLowerCase().includes(searchLower)) ||
          (c.name && c.name.toLowerCase().includes(searchLower))
        );
      }
    } else if (fieldName === 'parentLocation') {
      // Special handling for parent location dropdown
      if (!searchTerm || searchTerm.trim() === '') {
        this.dropdownStates[fieldName].filteredLocations = [...this.parentLocations];
      } else {
        const searchLower = searchTerm.toLowerCase();
        this.dropdownStates[fieldName].filteredLocations = this.parentLocations.filter(loc => 
          (loc.name && loc.name.toLowerCase().includes(searchLower)) ||
          (loc.locationCode && loc.locationCode.toLowerCase().includes(searchLower))
        );
      }
    } else {
  ...
  selectOption(fieldName: string, option: string): void {
    if (fieldName.startsWith('filter-')) {
      ...
      this.locationForm.patchValue({ currency: option });
    } else {
  ...
  getDisplayValue(fieldName: string): string {
    if (fieldName === 'currency') {
      const currencyCode = this.locationForm.get('currency')?.value;
      if (currencyCode) {
        // Simple: show code if currency found, otherwise just show code
        const currency = this.currencies.find(c => c.code === currencyCode);
        if (currency) {
          return `${currency.code} - ${currency.name}`;
        }
        return currencyCode; // Just show the code if currency not found in list
      }
  ...
  getParentLocationDisplay(): string {
    const parentId = this.locationForm.get('parentLocationId')?.value;
    if (!parentId) return '';
    const parent = this.parentLocations.find(loc => loc.id === parentId);
    return parent ? parent.name : '';
  }

  getParentLocationName(parentId: string | null | undefined): string {
    if (!parentId) return '';
    const parent = this.items.find(loc => loc.id === parentId);
    return parent ? parent.name : '';
  }
   ...
   * @param fieldName - Field name (e.g., 'filter-status', 'currency', etc.)
   */
  clearDropdownValue(fieldName: string, event?: Event): void {
    ...
    if (fieldName.startsWith('filter-')) {
      ...
      this.locationForm.patchValue({ currency: '' });
    } else if (fieldName === 'parentLocation') {
      this.locationForm.patchValue({ parentLocationId: '' });
    } else {
      // Form dropdowns
      this.formData[fieldName as keyof typeof this.formData] = '' as any;
      this.locationForm.patchValue({ [fieldName]: '' });
    }
  ...
  hasDropdownValue(fieldName: string): boolean {
    if (fieldName.startsWith('filter-')) {
      ...
      return !!this.locationForm.get('currency')?.value;
    } else if (fieldName === 'parentLocation') {
      return !!this.locationForm.get('parentLocationId')?.value;
    } else {
      // Form dropdowns
  ...
  toggleActiveStatus(item: Location): void {
    const newStatus = !item.isActive;
    this.orgService.updateLocation(item.id, { ...item, isActive: newStatus }).subscribe({
      next: () => {
        this.notification.success('Success', `Location ${newStatus ? 'activated' : 'deactivated'} successfully!`);
        this.resetSortToDefault();
        this.loadData();
        this.loadStatistics();
      },
      error: () => {
        this.notification.error('Error', 'Failed to update location status');
      }
    });
  }
