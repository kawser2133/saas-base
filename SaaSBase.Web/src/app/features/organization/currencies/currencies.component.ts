...
import { Currency, OrganizationService, ImportJobStatus, ExportJobStatus, ImportExportHistory, ImportExportHistoryResponse } from '../../../core/services/organization.service';
...
export class CurrenciesComponent implements OnInit, OnChanges, OnDestroy {
  // Utility references
  Math = Math;
  document = document;
  parseInt = parseInt;
  ...
  selectedItem: Currency | null = null;

  // Filters
  filters: any = { createdFrom: '', createdTo: '', status: '', code: '' };
  filterFormData: any = { createdFrom: '', createdTo: '', status: '', code: '' };
  showFilters = false;
  searchQuery = '';
  ...
  private initializeForm(): void {
    this.currencyForm = this.fb.group({
      ...
      decimalPlaces: [2, [Validators.min(0), Validators.max(8)]],
      isDefault: [false],
      isActive: [true]
    });
  }
  ...
  initializeDropdownStates(): void {
    const fields = ['filter-status', 'filter-code', 'items-per-page'];
  ...
  loadData(): void {
    ...
    const sub = this.orgService.getCurrencies(params).subscribe({
      next: (response: any) => {
        const items = Array.isArray(response?.items) ? response.items : (Array.isArray(response) ? response : []);
        this.items = items.map((item: Currency) => ({
          ...item,
          showDropdown: false,
          dropdownUp: false
        }));
      ...
      error: (error) => {
        ...
        this.notification.error('Error', 'Failed to load currencies. Please try again.');
        this.isLoading = false;
        this.loading = false;
        this.cdr.detectChanges();
      }
  ...
  loadFilterOptions(): void {
    ...
    this.orgService.getCurrencyCodes().subscribe({
      next: (codes) => {
  ...
  clearFilters(): void {
    this.searchQuery = '';
    this.searchTerm = '';
    this.filters = { createdFrom: '', createdTo: '', status: '', code: '' };
    this.filterFormData = { createdFrom: '', createdTo: '', status: '', code: '' };
  ...
  openEditDialog(item: Currency): void {
  ...
  openViewDialog(item: Currency): void {
    // Load full currency details with metadata
    const sub = this.orgService.getCurrency(item.id).subscribe({
      next: (fullCurrency) => {
        this.selectedItem = fullCurrency;
  ...
  openEditDialogFromView(item: Currency): void {
    this.showViewDialog = false;
    this.openEditDialog(item);
  }

  exportCurrencyDetailsAsPDF(): void {
    if (!this.selectedItem) {
      this.notification.warning('Warning', 'No currency selected to export');
      return;
    }

    const modalBody = document.querySelector('.modal-overlay .modal .modal-body') as HTMLElement;
    const currencyDetailsCards = document.querySelector('.modal-overlay .modal .profile-form') as HTMLElement;

    if (!modalBody || !currencyDetailsCards) {
    ...
    setTimeout(() => {
      html2canvas(currencyDetailsCards, {
        scale: 2,
        useCORS: true,
        allowTaint: true,
        ...
        scrollX: 0,
        scrollY: 0,
        windowWidth: currencyDetailsCards.scrollWidth,
        windowHeight: currencyDetailsCards.scrollHeight
      }).then(canvas => {
        ...
        pdf.save(`currency-details-${this.selectedItem?.code.replace(/\s+/g, '-').toLowerCase()}.pdf`);
  ...
  openDeleteDialog(items: Currency[]): void {
  ...
  cloneSelected(): void {
    ...
    if (selectedIds.length === 0) {
      this.notification.warning('Warning', 'Please select at least one currency to clone');
      return;
    }

    this.notification.info('Info', 'Cloning currencies...');
    this.orgService.cloneMultipleCurrencies(selectedIds).subscribe({
      next: (response) => {
        this.notification.success('Success', response.message || `${selectedIds.length} currency(ies) cloned successfully`);
        this.selectedItems.clear();
        this.resetSortToDefault();
        this.loadData();
      },
      error: (error) => {
        this.notification.error('Error', error.error?.message || 'Failed to clone currencies');
      }
    });
  }

  cloneCurrency(item: Currency): void {
    this.notification.info('Info', 'Cloning currency...');
    this.orgService.cloneMultipleCurrencies([item.id]).subscribe({
      next: (response) => {
        this.notification.success('Success', response.message || 'Currency cloned successfully');
        this.resetSortToDefault();
        this.loadData();
      },
      error: (error) => {
        this.notification.error('Error', error.error?.message || 'Failed to clone currency');
      }
    });
  }

  closeDropdown(item: Currency): void {
  ...
  saveItem(): void {
    ...
    if (this.dialogMode === 'add') {
      this.orgService.createCurrency(formValue).subscribe({
        next: () => {
          this.closeDialogs();
          this.notification.success('Success', 'Currency created successfully!');
          this.isSubmitting = false;
          this.resetSortToDefault();
          // Refresh currency service cache
          this.currencyService.refreshCurrencies();
          this.loadData();
          this.loadStatistics();
        },
        error: (error) => {
          this.notification.error('Error', error.error?.message || 'Failed to create currency');
          this.isSubmitting = false;
        }
      });
    } else if (this.selectedItem?.id) {
      this.orgService.updateCurrency(this.selectedItem.id, formValue).subscribe({
        next: () => {
          this.closeDialogs();
          this.notification.success('Success', 'Currency updated successfully!');
          this.isSubmitting = false;
          this.resetSortToDefault();
          // Refresh currency service cache
          this.currencyService.refreshCurrencies();
        ...
        error: (error) => {
          this.notification.error('Error', error.error?.message || 'Failed to update currency');
          this.isSubmitting = false;
        }
      });
    }
  ...
  confirmDelete(): void {
    ...
    if (itemsToDelete.length === 1) {
      this.orgService.deleteCurrency(itemsToDelete[0]).subscribe({
        next: () => {
          this.selectedItems.clear();
          this.closeDialogs();
          this.notification.success('Success', 'Currency deleted successfully!');
          this.resetSortToDefault();
          // Refresh currency service cache
          this.currencyService.refreshCurrencies();
          this.loadData();
        ...
        error: (error) => {
          this.notification.error('Error', error.error?.message || 'Failed to delete currency');
        }
      });
    } else {
      let deletedCount = 0;
      ...
      itemsToDelete.forEach(id => {
        this.orgService.deleteCurrency(id).subscribe({
          next: () => {
            deletedCount++;
            if (deletedCount === totalCount) {
              ...
              // Refresh currency service cache
              this.currencyService.refreshCurrencies();
              this.notification.success('Success', `${totalCount} currencies deleted successfully`);
              this.resetSortToDefault();
              this.loadData();
              this.loadStatistics();
            }
          ...
          error: () => {
            ...
            if (deletedCount === totalCount) {
              this.selectedItems.clear();
              this.closeDialogs();
              this.notification.warning('Warning', 'Some currencies could not be deleted');
              this.resetSortToDefault();
              this.loadData();
              this.loadStatistics();
            }
  ...
  exportCurrencies(format: 'excel' | 'csv' | 'pdf' | 'json'): void {
    ...
    this.orgService.startCurrencyExportAsync(params).subscribe({
      next: (response) => {
  ...
  private pollExportJobStatus(): void {
    ...
    const intervalId = setInterval(() => {
      ...
      this.orgService.getCurrencyExportJobStatus(this.exportJobId).subscribe({
  ...
  private downloadCompletedExport(jobId: string, format: string): void {
    this.orgService.downloadCurrencyExport(jobId).subscribe({
      next: (blob: Blob) => {
        ...
        link.href = url;
        link.download = `currencies_export_${new Date().toISOString().split('T')[0]}.${extension}`;
  ...
  downloadTemplate(): void {
    this.orgService.getCurrencyTemplate().subscribe({
      next: (response: Blob) => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = 'currencies_import_template.xlsx';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
  ...
  importData(): void {
    ...
    // Start async job
    this.orgService.startCurrencyImportAsync(formData).subscribe({
      next: (res) => {
  ...
  private pollImportJobStatus(): void {
    ...
    const intervalId = setInterval(() => {
      ...
      this.orgService.getCurrencyImportJobStatus(this.importJobId).subscribe({
        next: (status: ImportJobStatus) => {
          ...
          if (status.status === 'Completed' || status.status === 'Failed') {
            ...
            if (status.status === 'Completed') {
              this.notification.success('Import Completed', `Imported ${status.successCount} currencies${status.skippedCount ? `, skipped ${status.skippedCount}` : ''}${status.errorCount ? `, errors ${status.errorCount}` : ''}.`);
  ...
  downloadErrorReport(): void {
    ...
    this.orgService.getCurrencyImportErrorReport(this.errorReportId).subscribe({
      next: (response: Blob) => {
  ...
  loadHistory(): void {
    ...
    this.orgService.getCurrencyHistory(this.historyType, this.historyPage, this.historyPageSize).subscribe({
      next: (response) => {
  ...
  downloadExportFromHistory(historyId: string): void {
    ...
    this.orgService.downloadCurrencyExport(history.jobId).subscribe({
      next: (blob: Blob) => {
        // Determine file extension based on format
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = history.fileName || `currencies_export_${new Date().toISOString().split('T')[0]}.${extension}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
  ...
  downloadErrorReportForHistory(historyId: string): void {
    ...
    this.orgService.getCurrencyImportErrorReport(history.errorReportId).subscribe({
      next: (response: Blob) => {
        const blob = new Blob([response], { type: 'text/csv' });
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
  ...
  // Dropdown methods
  isDropdownOpen(item: Currency): boolean {
    return !!(item as any).showDropdown;
  }

  toggleDropdown(item: Currency, event?: MouseEvent): void {
    if (event) event.stopPropagation();
    this.exportDropdownOpen = false;
    this.items.forEach((dataItem: Currency) => {
  ...
  toggleActiveStatusAndClose(item: Currency): void {
    this.toggleActiveStatus(item);
    (item as any).showDropdown = false;
  }

  deleteItemAndClose(item: Currency): void {
    this.openDeleteDialog([item]);
    (item as any).showDropdown = false;
  }

  closeAllDropdowns(): void {
    this.items.forEach((item: Currency) => {
      (item as any).showDropdown = false;
    });
   ...
   * @param fieldName - Field name (e.g., 'filter-status', 'filter-code', etc.)
   */
  ...
  toggleActiveStatus(item: Currency): void {
    const newStatus = !item.isActive;
    this.orgService.updateCurrency(item.id, { ...item, isActive: newStatus }).subscribe({
      next: () => {
        this.notification.success('Success', `Currency ${newStatus ? 'activated' : 'deactivated'} successfully!`);
        this.resetSortToDefault();
        this.loadData();
        this.loadStatistics();
      },
      error: () => {
        this.notification.error('Error', 'Failed to update currency status');
      }
    });
  }
